...
const randomImages = () => {
  while (imagesArray.length < 8) {
    ...
    if (!imagesArray.includes(randomValue)) {
      imagesArray.push(randomValue);
    }
  }
  imagesArray.push(9);
};
...
const gridGenerator = () => {
  ...
  for (let i = 0; i < 3; i++) {
    for (let j = 0; j < 3; j++) {
      ...
      div.innerHTML = `<img src="./images/image_${
        imagesArray[count]
      }.jpg" class="image ${
        imagesArray[count] == 9 ? "target" : ""
      }" data-index="${imagesArray[count]}"/>`;
      count += 1;
...
const selectImage = (e) => {
  ...
  if (checkAdjacent(row1, row2, col1, col2)) {
    ...
    //Array swaps
    let currentArrayIndex = imagesArray.indexOf(currentIndex);
    let targetArrayIndex = imagesArray.indexOf(targetIndex);
    [imagesArray[currentArrayIndex], imagesArray[targetArrayIndex]] = [
      imagesArray[targetArrayIndex],
      imagesArray[currentArrayIndex],
    ];
    ...
    if (imagesArray.join("") == "123456789") {
      setTimeout(() => {
        ...
        startBtn.innerText = "RestartGame";
      }, 1000);
    }
